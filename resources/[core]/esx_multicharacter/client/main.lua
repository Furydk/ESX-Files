...
if ESX.GetConfig().Multichar then
	...
	function CharacterOptions(Characters, slots, SelectedCharacter)
		...
		lib.registerContext({
			id = 'ox_options',
			title = TranslateCap('character',
				Characters[SelectedCharacter].firstname .. " " .. Characters[SelectedCharacter].lastname),
			options = option,
			canClose = false
		})
			...
			if Characters[k].disabled then
				...
				option[#option + 1] = {
					...
					metadata = {
						...
						{ label = 'Cash',          value = formatNumber(v.money) .. ' KR' },
						{ label = 'Bank',          value = formatNumber(v.bank) .. ' KR' },
					},
					onSelect = function(_, SelectedCharacter)
						SelectedCharacter = v.id
						CharacterOptions(Characters, slots, SelectedCharacter)
						...
						lib.requestAnimDict('friends@frj@ig_1')
						TaskPlayAnim(PlayerPedId(), 'friends@frj@ig_1', 'wave_b', 2.0, 1.0, 6500, 16, 0, false, false,
							false)
						local playerPed = PlayerPedId()
						SetPedAoBlobRendering(playerPed, true)
						ResetEntityAlpha(playerPed)
