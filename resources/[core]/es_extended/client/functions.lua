...
function ESX.ShowNotification(message, notifyType, length)
    lib.notify({ type = notifyType, length = length, description = message })
end
...
function ESX.Game.SpawnObject(object, coords, cb, networked)
    local obj = CreateObject(ESX.Streaming.RequestModel(object), coords.x, coords.y, coords.z,
        networked == nil or networked, false, true)
...
function ESX.Game.SpawnVehicle(vehicleModel, coords, heading, cb, networked)
    ...
    if dist > 424 then -- Onesync infinity Range (https://docs.fivem.net/docs/scripting-reference/onesync/)
        ...
        return error(("Resource ^5%s^7 Tried to spawn vehicle on the client but the position is too far away (Out of onesync range).")
...
function ESX.Game.RaycastScreen(depth, ...)
    ...
    return target,
        ESX.Game.GetShapeTestResultSync(StartShapeTestLosProbe(origin.x, origin.y, origin.z, target.x, target.y, target
            .z, ...))
end
